...
import {APIResponseType} from "../../../shared/api/api";
import {FilterType, usersActions} from "../users_reducer";
import {createAppAsyncThunk} from "../../../shared/lib/redux";

export const requestUsers = createAppAsyncThunk('users/requestUsers', async (payload: {
	currentPage: number,
	pageSize: number,
	filter: FilterType
}, {dispatch}) => {
...
export const follow = createAppAsyncThunk('users/follow', async (userId: number, {dispatch}) => {
	await _followUnfollowFlow(dispatch, userId, usersAPI.follow.bind(usersAPI), usersActions.followSuccess)
})

export const unfollow = createAppAsyncThunk('users/unfollow', async (userId: number, {dispatch}) => {
